    ...
    <main>
          ...
          {activeCodes.length === 0 ? (
            <article className="content-card md:col-span-2 lg:col-span-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h3 className="text-xl font-bold text-white">No verified active codes yet</h3>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {toolCards.map((tool) => (
            <Link key={`${tool.href}-${tool.title}`} href={tool.href} className="content-card">
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 lg:grid-cols-2">
          <div>
            <SectionHeader
              ...
              copy="Use these guides when you need first-shift help, safer routes, source checks, endings, badges, or survival habits."
            />
          ...
          <div>
            <SectionHeader
              ...
              copy="Monsters, endings, badges, store areas, items, codes, and puzzle notes should be split into wiki entries when research confirms them."
            />
            <div className="mt-6 grid gap-3">
              {wikiCards.map((item) => (
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          {officialLinks.map((link) => {
            ...
            return link.href.startsWith("/") ? (
              <Link key={`${link.href}-${link.title}`} href={link.href} className="content-card">
                {content}
              </Link>
            ) : (
              <a key={`${link.href}-${link.title}`} href={link.href} className="content-card" target="_blank" rel="noreferrer">
                {content}
              </a>
            );
          })}
