...
const goals = ["codes-first", "sources", "tasks", "endings"] as const;
...
export function CalculatorTool() {
  ...
  const result = useMemo(() => {
    ...
    if (goal === "sources" && stage < 3) {
      return "Confirm the official Roblox page first, then check whether any Trello, Discord, or wiki link is creator-owned before trusting it.";
    }
    if (goal === "tasks") {
      return hasBoost ? "Use the checklist during the shift and mark each task, danger sign, and exit note as you verify it." : "Start with the Roblox page, codes status, and a simple task route before following deeper walkthrough claims.";
    }
    if (goal === "endings") {
      return "Do not treat endings, badges, or secrets as final until a current video, in-game test, or official source confirms the route.";
    }
        ...
        <div className="mt-6 grid gap-5">
          ...
          <label className="grid gap-2">
            ...
            <select value={goal} onChange={(event) => setGoal(event.target.value as (typeof goals)[number])} className="rounded-lg border border-white/10 bg-[#111113] px-3 py-3 text-white">
              ...
              <option value="sources">Confirm official links</option>
              <option value="tasks">Plan shift tasks</option>
              <option value="endings">Check endings or badges</option>
            </select>
          </label>
          <button
            type="button"
