        ...
        <div className="grid gap-8 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
          ...
          <div>
            ...
            <div className="mt-3 grid gap-2 text-sm text-white/62">
              ...
              <Link href="/calculator" className="hover:text-white">Checklist</Link>
            ...
            <div className="mt-3 grid gap-2 text-sm text-white/62">
              {officialLinks.map((link) => (
                link.href.startsWith("/") ? (
                  <Link key={`${link.href}-${link.title}`} href={link.href} className="hover:text-white">
                    {link.title}
                  </Link>
                ) : (
                  <a key={`${link.href}-${link.title}`} href={link.href} target="_blank" rel="noreferrer" className="hover:text-white">
                    {link.title}
                  </a>
                )
              ))}
